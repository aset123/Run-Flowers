window.addEventListener("load", () => {
  ...
  const character = {
    x: canvas.width * 0.1,
    y: groundY - Math.max(canvas.height * 0.2, 100),
    width: Math.max(canvas.width * 0.12, 60),
    height: Math.max(canvas.height * 0.2, 100),
    vy: 0,
    jumping: false
  };
  ...
  function createFlower() {
    flowers.push({
      x: canvas.width,
      y: groundY - Math.max(canvas.height * 0.18, 80),
      width: Math.max(canvas.width * 0.08, 50),
      height: Math.max(canvas.height * 0.18, 80)
    });
  }
  ...
  function createObstacle() {
    obstacles.push({
      x: canvas.width,
      y: groundY - Math.max(canvas.height * 0.18, 80),
      width: Math.max(canvas.width * 0.06, 40),
      height: Math.max(canvas.height * 0.18, 80)
    });
  }
  ...
  document.addEventListener("touchstart", () => {
    if (!character.jumping && !gameOver) {
      character.vy = -Math.max(canvas.height * 0.05, 40);
